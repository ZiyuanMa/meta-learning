...
#         print(x_spts[0].shape)
        ...
        y_spts = np.array(y_spts).astype(np.long).reshape(batch_size, setsz)
        # [b, qrysz = n_way * k_query, 1, 84, 84]
        x_qrys = np.array(x_qrys).astype(np.float32).reshape(batch_size, querysz, 1, resize, resize)
        y_qrys = np.array(y_qrys).astype(np.long).reshape(batch_size, querysz)
#         print(x_qrys.shape)
...
#         print('hello')
        ...
        loss = torch.tensor(loss_list_qry).detach().numpy() / ( task_num)
        return accs, loss
